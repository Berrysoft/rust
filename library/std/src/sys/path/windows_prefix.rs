...
/// Returns the parsed prefix and possible pre-prefix length on Cygwin.
/// Usually the pre-prefix length is 0, but `//./` & `//../` are treated specially
/// as equivalent to `//`, and in that case the pre-prefix length is 2 for `./`
/// and 3 for `../`.
pub fn parse_prefix(path: &OsStr) -> Option<(usize, Prefix<'_>)> {
    ...
    if let Some(parser) = parser.strip_prefix(r"\\") {
        ...
        // Cygwin treats //./ & //../ as //
        let (pre_prefix_len, new_parser) =
            if cfg!(target_os = "cygwin") && path.as_encoded_bytes().starts_with(b"//.") {
                let mut pre_prefix_len = 0;
                let mut path = &path.as_encoded_bytes()[2..];
                loop {
                    // Only search for `/`.
                    match path.iter().position(|&x| x == b'/') {
                        Some(separator_start) => {
                            let component = &path[..separator_start];
                            if matches!(component, b"." | b"..") {
                                let separator_end = separator_start + 1;
                                path = &path[separator_end..];
                                pre_prefix_len += separator_end;
                            } else {
                                break;
                            }
                        }
                ...
                (pre_prefix_len, Some(PrefixParser::<6>::new(path)))
            } else {
                (0, None)
            };
        let parser = new_parser.as_ref().map(|parser| parser.as_slice()).unwrap_or(parser);

        // The meaning of verbatim paths can change when they use a different
        // separator.
        let prefix = if let Some(parser) = parser.strip_prefix(r"?\")
